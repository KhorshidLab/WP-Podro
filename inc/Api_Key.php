    ...
    */
    private static function validate_api_key(string $email, string $password) {
		...
		if ( !$response ) {
			add_action( 'admin_notices', function (){
				echo wp_kses_post('<div class="notice notice-error is-dismissible">
					<p>'. esc_html__( 'کاربر غیر مجاز است.', 'podro-wp' ) .'</p>
				</div>');
			} );
		}
		...
		if ($http_code == 500) {
			add_action( 'admin_notices', function () {
				echo wp_kses_post('<div class="notice notice-error is-dismissible">
						<p>'. esc_html__( 'پادرو در حال حاضر پاسخی نمی دهد. لطفا بعدا دوباره امتحان کنید', 'podro-wp' ) .'</p>
					</div>');
			} );
		} else if ($http_code == 401) {
			add_action( 'admin_notices', function () {
				echo wp_kses_post('<div class="notice notice-error is-dismissible">
					<p>'. esc_html__( 'کلید Podro API نامعتبر است. لطفا دوباره تلاش کنید.', 'podro-wp' ) .'</p>
				</div>');
			} );
